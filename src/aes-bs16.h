...
{
    // ftop
    const uint64_t Z6   = u[1] ^ u[2];
    const uint64_t Q12  = Z6 ^ u[3];
    const uint64_t Q11  = u[4] ^ u[5];
    const uint64_t Q0   = Q12 ^ Q11;
    const uint64_t Z9   = u[0] ^ u[3];
    const uint64_t Z80  = u[4] ^ u[6];
    const uint64_t Q1   = Z9 ^ Z80;
    const uint64_t Q7   = Z6 ^ u[7];
    const uint64_t Q2   = Q1 ^ Q7;
    const uint64_t Q3   = Q1 ^ u[7];
    const uint64_t Q13  = u[5] ^ Z80;
    const uint64_t Q5   = Q12 ^ Q13;
    const uint64_t Z66  = u[1] ^ u[6];
    const uint64_t Z114 = Q11 ^ Z66;
    const uint64_t Q6   = u[7] ^ Z114;
    const uint64_t Q8   = Q1 ^ Z114;
    const uint64_t Q9   = Q7 ^ Z114;
    const uint64_t Q10  = u[2] ^ Q13;
    const uint64_t Q16  = Z9 ^ Z66;
    const uint64_t Q14  = Q16 ^ Q13;
    const uint64_t Q15  = u[0] ^ u[2];
    const uint64_t Q17  = Z9 ^ Z114;
    const uint64_t Q4   = u[7];

    // mulx.a
    uint64_t T20 = NAND(Q6, Q12);
    uint64_t T21 = NAND(Q3, Q14);
    uint64_t T22 = NAND(Q1, Q16);
    uint64_t T10 = (NOR(Q3, Q14) ^ NAND(Q0, Q7));
    uint64_t T11 = (NOR(Q4, Q13) ^ NAND(Q10, Q11));
    uint64_t T12 = (NOR(Q2, Q17) ^ NAND(Q5, Q9));
    uint64_t T13 = (NOR(Q8, Q15) ^ NAND(Q2, Q17));
    uint64_t X0  = T10 ^ (T20 ^ T22);
    uint64_t X1  = T11 ^ (T21 ^ T20);
    uint64_t X2  = T12 ^ (T21 ^ T22);
    uint64_t X3  = T13 ^ (T21 ^ NAND(Q4, Q13));

    // s1.a
    uint64_t T0 = NAND(X0, X2);
    uint64_t T1 = NOR(X1, X3);
    uint64_t T2 = XNOR(T0, T1);
    uint64_t Y0 = MUX(X2, T2, X3);
    uint64_t Y2 = MUX(X0, T2, X1);
    uint64_t T3 = MUX(X1, X2, ~0);
    uint64_t Y1 = MUX(T2, X3, T3);
    uint64_t T4 = MUX(X3, X0, ~0);
    uint64_t Y3 = MUX(T2, X1, T4);

    const uint64_t T5  = MUX(X0, T0, X3);
    const uint64_t Y23 = MUX(X1, T5, X0);
    const uint64_t T6  = NMUX(T3, X2, X3);
    const uint64_t Y01 = NMUX(T0, T6, X3);
    const uint64_t Y02 = Y2 ^ Y0;
    const uint64_t Y13 = Y3 ^ Y1;
    const uint64_t Y00 = Y01 ^ Y23;

    // muln.a
    uint64_t N0  = NAND(Y01, Q11);
    uint64_t N1  = NAND(Y0, Q12);
    uint64_t N2  = NAND(Y1, Q0);
    uint64_t N3  = NAND(Y23, Q17);
    uint64_t N4  = NAND(Y2, Q5);
    uint64_t N5  = NAND(Y3, Q15);
    uint64_t N6  = NAND(Y13, Q14);
    uint64_t N7  = NAND(Y00, Q16);
    uint64_t N8  = NAND(Y02, Q13);
    uint64_t N9  = NAND(Y01, Q7);
    uint64_t N10 = NAND(Y0, Q10);
    uint64_t N11 = NAND(Y1, Q6);
    uint64_t N12 = NAND(Y23, Q2);
    uint64_t N13 = NAND(Y2, Q9);
    uint64_t N14 = NAND(Y3, Q8);
    uint64_t N15 = NAND(Y13, Q3);
    uint64_t N16 = NAND(Y00, Q1);
    uint64_t N17 = NAND(Y02, Q4);

    // fbot
    const uint64_t H0  = N3 ^ N8;
    const uint64_t H1  = N5 ^ N6;
    const uint64_t H2  = XNOR(H0, H1);
    const uint64_t H3  = N1 ^ N4;
    const uint64_t H4  = N9 ^ N10;
    const uint64_t H5  = N13 ^ N14;
    const uint64_t H6  = N15 ^ H4;
    const uint64_t H7  = N0 ^ H3;
    const uint64_t H8  = N17 ^ H5;
    const uint64_t H9  = N3 ^ H7;
    const uint64_t H10 = N15 ^ N17;
    const uint64_t H11 = N9 ^ N11;
    const uint64_t H12 = N12 ^ N14;
    const uint64_t H13 = N1 ^ N2;
    const uint64_t H14 = N5 ^ N16;
    const uint64_t H15 = N7 ^ H11;
    const uint64_t H16 = H10 ^ H11;
    const uint64_t H17 = N16 ^ H8;
    const uint64_t H18 = H6 ^ H8;
    const uint64_t H19 = H10 ^ H12;
    const uint64_t H20 = N2 ^ H3;
    const uint64_t H21 = H6 ^ H14;
    const uint64_t H22 = N8 ^ H12;
    const uint64_t H23 = H13 ^ H15;
