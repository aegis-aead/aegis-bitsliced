...
test "aegis256 - RFC test vector 3" {
    const key = [_]u8{ 0x10, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    const nonce = [_]u8{ 0x10, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    const ad = [_]u8{ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 };
    const msg = [_]u8{ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f };
    const expected_ct = [_]u8{ 0xf3, 0x73, 0x07, 0x9e, 0xd8, 0x4b, 0x27, 0x09, 0xfa, 0xee, 0x37, 0x35, 0x84, 0x58, 0x5d, 0x60, 0xac, 0xcd, 0x19, 0x1d, 0xb3, 0x10, 0xef, 0x5d, 0x8b, 0x11, 0x83, 0x3d, 0xf9, 0xde, 0xc7, 0x11 };
    const expected_tag128 = [_]u8{ 0x8d, 0x86, 0xf9, 0x1e, 0xe6, 0x06, 0xe9, 0xff, 0x26, 0xa0, 0x1b, 0x64, 0xcc, 0xbd, 0xd9, 0x1d };
    const expected_tag256 = [_]u8{ 0xb7, 0xd2, 0x8d, 0x0c, 0x3c, 0x0e, 0xbd, 0x40, 0x9f, 0xd2, 0x2b, 0x44, 0x16, 0x05, 0x03, 0x07, 0x3a, 0x54, 0x74, 0x12, 0xda, 0x08, 0x54, 0xbf, 0xb9, 0x72, 0x30, 0x20, 0xda, 0xb8, 0xda, 0x1a };
    ...
    var ret = aegis.aegis256_encrypt_detached(&ct, &tag128, 16, &msg, msg.len, &ad, ad.len, &nonce, &key);
    try testing.expectEqual(ret, 0);
    try testing.expectEqualSlices(u8, &expected_ct, &ct);
    try testing.expectEqualSlices(u8, &expected_tag128, &tag128);

    ret = aegis.aegis256_encrypt_detached(&ct, &tag256, 32, &msg, msg.len, &ad, ad.len, &nonce, &key);
    try testing.expectEqual(ret, 0);
    try testing.expectEqualSlices(u8, &expected_ct, &ct);
    try testing.expectEqualSlices(u8, &expected_tag256, &tag256);

    ret = aegis.aegis256_decrypt_detached(&dec_msg, &ct, ct.len, &tag256, 32, &ad, ad.len, &nonce, &key);
    try testing.expectEqual(ret, 0);
